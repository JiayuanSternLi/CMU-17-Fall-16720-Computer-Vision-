...
[b_row,b_col,depth]=size(bases);
vect_bases=reshape(bases,[b_row*b_col,depth]);
...
[baseEleNum,~]=size(vect_bases);
idenMat=zeros(baseEleNum,baseEleNum);
BBterm=idenMat-vect_bases*vect_bases';
stepDescentImgWithBase=BBterm*stepDescentImg;
Hessian=stepDescentImgWithBase'*stepDescentImgWithBase;
...
for i=1:iter
    ...
    % step7 Delta_p
    delta_p=Hessian\(stepDescentImgWithBase'*(BBterm*ErImg));
